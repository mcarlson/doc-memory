...
export class DocMemory {
    ...
    assertBackend() {
        if (!this.storage || !this.pipeline || !this.events) {
            throw new Error('DocMemory was constructed retriever-only (injected retriever, no storage config); ' +
    ...
    async index(filepath, source = 'directory') {
        this.assertBackend();
    ...
    async expand(chunkId, level = 'adjacent') {
        this.assertBackend();
    ...
    async startWatching(paths, glob) {
        this.assertBackend();
        const watcher = new FileWatcher(this.pipeline, { paths, glob }, { source: 'directory' });
        await watcher.start();
