...
export function chunkTextWithMetadata(text, options = {}) {
    ...
    const headerPositions = new Set(); // Track positions to avoid duplicates
    ...
    const headerPositionSet = new Set();
    ...
    const pageBreakPositions = new Set();
    ...
    while (start < cleanText.length) {
        ...
        if (end < cleanText.length) {
            ...
            const headerBreak = sortedHeaderBreaks.find(pos => pos > start + minChunkForHeaderBreak && pos <= end);
            if (headerBreak !== undefined) {
                end = headerBreak;
            }
            else {
                // 2. Look for page break within normal range
                const pageBreak = [...pageBreakPositions].find(pos => pos > minBreak && pos <= end);
